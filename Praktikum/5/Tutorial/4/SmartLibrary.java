...
public class SmartLibrary<T extends Comparable<T>> implements Iterable<T>, Searchable<T> {
    ...
    public void sortByRatingDesc() {
        ...
        books.sort((b1,b2) -> Double.compare(((Book) b2).getRating(), ((Book) b1).getRating()));
    }
    ...
    public T findByAuthor(String author) {
        ...
        books.sort(Comparator.comparing(b -> ((Book) b).getAuthor(), String.CASE_INSENSITIVE_ORDER));
        Comparator<T> c = (a, b) -> ((Book) a).getAuthor().compareToIgnoreCase(((Book) b).getAuthor());
        T key = (T) new Book(null, author, null, 0, 0);
