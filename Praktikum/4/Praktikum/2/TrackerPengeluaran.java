public class TrackerPengeluaran {
    ...
    public void setCatatan(int minggu, int kategori, int nominal, String deskripsi) {
        ...
        int idxM = minggu-1;
        if (idxM >= 0 && idxM < 4 && kategori >= 0 && kategori < 3) {
            matriks[idxM][kategori] = new Catatan(nominal, deskripsi);
        } else {
    ...
    public int totalPerMinggu(int minggu) {
        ...
        int total = 0;
        for (int j = 0; j < 3; j++) {
            if (matriks[idxM][j] != null) {
                total += matriks[idxM][j].getNominal();
            }
        }
        return total;
    }
    ...
    public void cetakLaporan() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 3; j++) {
                if (matriks[i][j] != null) {
                    System.out.print(matriks[i][j].getNominal() + " (" + matriks[i][j].getDeskripsi() + ")\t");
                } else {
